...
`ANTIGRAVITY_DIR` / `COPILOT_DIR` at directories present on the host. For a
public **demo deploy** (e.g. on Vercel), sample transcripts are committed under
`demo-data/` and regenerated by:

```bash
node scripts/generate-fake-data.mjs    # writes ~45 Claude sessions + ~25 Codex rollouts + ~20 OpenCode sessions + ~20 Antigravity transcripts + ~35 Copilot Chat sessions over 30 days
```

This produces five parallel trees that match the adapters' on-disk shapes:
...
- `demo-data/copilot/workspaceStorage/<hash>/chatSessions/<id>.jsonl` (+ `workspace.json` folder maps) and `demo-data/copilot/globalStorage/emptyWindowChatSessions/<id>.jsonl` — fake GitHub Copilot Chat sessions (token-bearing, multi-vendor: Anthropic / OpenAI / Gemini / local OLLAMA)
...
1. Push the repo (the `demo-data/` directory and `next.config.ts`'s
   ...
   dot-dirs, the `*.db` / `*.db-wal` / `*.db-shm` files, and the Copilot
   `chatSessions` JSONL + `workspace.json` folder maps).
...
3. Deploy. The dashboard renders the fake data across every panel — the
   overview shows all five agents' cards plus combined token/cost panels
   (Claude + Codex + OpenCode + Copilot) and activity contributions (Antigravity),
   and `/claude`, `/codex`, `/opencode`, `/copilot`, `/antigravity` show each
   agent scoped.

For a deploy backed by your **real** data, copy your `~/.claude/projects/`,
`~/.codex/`, `~/.local/share/opencode/`, `~/.gemini/antigravity-ide/brain/`, and
`~/Library/Application Support/Code/User/` (Copilot Chat) trees into the repo
(or a private sibling) and point the env vars at them the same way — but note
that exposes your real usage transcripts to anyone who can read the deploy.
